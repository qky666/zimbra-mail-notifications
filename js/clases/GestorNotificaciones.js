class GestorNotificaciones {
	...
	**************************************************************************/
	...
	static limpiarNotificacion (callback) {
	  chrome.notifications.clear(ID_NOTIF_CORREO, callback)
	}
			...
			OpcionesStorage.getOpciones(function(opciones) {
				...
				ZimbraNavegador.zimbraActivo(function (activo) { 
					...
					if(servidor.error) {
						...
						chrome.notifications.create(ID_NOTIF_CORREO, notificationOptions, function (notificationId) {} )
						return
					}
	...
	static hayNotificacionActiva (callback) {
		chrome.notifications.getAll(function (notifications) {
			if (notifications && notifications[ID_NOTIF_CORREO]) {
				callback(true)
				return
			}
			callback(false)
